...
public class RedisLock {
    ...
    static {
        ...
        sb.append("if redis.call(\"get\",KEYS[1]) == ARGV[1] "); // 取到key里面的值，如果相等说明是我加的锁，删除释放
        sb.append("then ");
        sb.append("    return redis.call(\"del\",KEYS[1]) ");
        sb.append("else ");
        sb.append("    return 0 "); // 否则返回0
        sb.append("end ");
    ...
    public boolean tryLock() {
        ...
        // 每个线程生成随机key,放到lockKey里面,解锁的时候需要使用
        lockValue = UUID.randomUUID().toString();

        // 请求锁超时时间，纳秒，在这个时间内没有竞争到锁就往下走，没有抢到商品
        long timeout = timeOut * 1000000;

        // 系统当前时间，纳秒
        long nowTime = System.nanoTime();

        // 分布式锁要自旋，一直阻塞的去竞争锁，要不然只有一个线程秒成功了,其他线程就结束了
...
//            if (OK.equalsIgnoreCase(result)) {
    ...
    private boolean set(final String key, final String value, final long seconds) {
        ...
        // 如果返回为true代表这个key在redis里面没有，并且返回true  setNX
        // 设置60秒，避免死锁，可以参考文档
